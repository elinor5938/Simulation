...
params["seed_mutation"]=random.seed(42) #seed for mutation creator
...
if path_flag==0:
    ...
    params["main_output_folder"] = os.path.join(main_path, "5000 pep sum sim july ten", "")
...
params={}
#params["probability_function"]=lambda x :1.0/(1+math.exp(x) +0.1) #arbitrary probability function
params["probability_function"]=lambda x :((-1)* math.exp(x) )/100
#params["probability_function"]=lambda x :(x+0.15)*10/12
params["seed"]=random.seed(42)
params["seed_mutation"]=random.seed(42) #seed for mutation creator
params["number of peptide for pred"]=5000
...
def simulation_process(peptide,column):
    ...
    while flag ==False:
        ...
        #appended_data .append(df1)
        print(appended_data)
        if len(appended_data.loc[appended_data['probabilty_res_MCMC'] == "True"])==10:  # there i decide the stop condition for this process
            flag=True
    return appended_data
#
# #
...
#         #appended_data .append(df1)
#         print(appended_data)
#         if len(appended_data.loc[appended_data['probabilty_res_MCMC'] == "True"])==5000:  # there i decide the stop condition for this process
#             flag=True
#     return appended_data
# #
# #
